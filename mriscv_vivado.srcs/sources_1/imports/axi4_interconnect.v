...
module axi4_interconnect #
	...
	*/
	parameter [slaves*sword-1:0] addr_mask = {32'h00000000,32'h0000001F,32'h000001FF,32'h000001FF,32'h00000001,32'h0FFFFFFF,32'h007FFFFF,32'h00000FFF},
	...
	reg [masters-1:0] dec_wrequests;
	reg [masters-1:0] dec_rrequests;
	...
	begin
		...
		begin
			...
			end else if(counter_rrequests == masters)
			begin
				counter_rrequests <= {numbit_masters{1'b0}};
			end else
			begin
	...
	begin
		...
		begin
			...
			end else if(counter_wrequests == masters)
			begin
				counter_wrequests <= {numbit_masters{1'b0}};
			end else
			begin
	...
	begin
		if (RST == 1'b0)
		begin
			dec_rrequests <= 1;		// RESET 
		end else 
		begin
			if (en_rrequests == 1'b0)
			begin
				dec_rrequests <= dec_rrequests;	// NOTHING 
			end else 
			begin
				dec_rrequests <= {dec_rrequests[masters-2:0], dec_rrequests[masters-1]};						// SHIFTING
			end
		end
	end
	...
	begin
		if (RST == 1'b0)
		begin
			dec_wrequests <= 1;		// RESET 
		end else 
		begin
			if (en_wrequests == 1'b0)
			begin
				dec_wrequests <= dec_wrequests;	// NOTHING 
			end else 
			begin
				dec_wrequests <= {dec_wrequests[masters-2:0], dec_wrequests[masters-1]};						// SHIFTING
			end
		end
	end
	...
	/*generate // NOT LATCHED THING
        for (unpk_idx=0; unpk_idx<(masters); unpk_idx=unpk_idx+1) begin 
            assign m_axi_awaddr_g[unpk_idx] = m_axi_awaddr[((sword)*unpk_idx+(sword-1)):((sword)*unpk_idx)];
            assign m_axi_araddr_g[unpk_idx] = m_axi_araddr[((sword)*unpk_idx+(sword-1)):((sword)*unpk_idx)];
        end 
    endgenerate*/
	...
	generate
		if(addressing) begin
			// Giant MUX
			assign awaddr = m_axi_awaddr_g[counter_wrequests];
	...
	wire slave_read_valid, slave_write_valid;
	...
	generate
		if(addressing) begin
			// Giant Encoder
			priencr #(.width(slaves)) priencr_raddrencr(.decode(slave_rdec1),.encode(slave_raddr),.valid(slave_read_valid));
			priencr #(.width(slaves)) priencr_waddrencr(.decode(slave_wdec1),.encode(slave_waddr),.valid(slave_write_valid));
	...
	wire slave_rvalid, slave_wvalid, slave_bvalid, slave_rready, slave_bready;
	generate
		if(addressing) begin
			// Giant MUX 
			assign slave_rvalid = s_axi_rvalid[slave_raddr];
			assign slave_bvalid = s_axi_bvalid[slave_waddr];
            assign slave_wvalid = s_axi_wvalid[slave_waddr];
            assign slave_bready = s_axi_bready[slave_waddr];
            assign slave_rready = s_axi_rready[slave_raddr];
		end else begin
			// tri-state buff bus
			wire [slaves-1:0] trib_rvalid;
            wire [slaves-1:0] trib_bvalid;
            wire [slaves-1:0] trib_rready;
            wire [slaves-1:0] trib_bready;
            wire [slaves-1:0] trib_wvalid;
			for (i = 0; i < slaves; i = i + 1) begin 
				assign trib_rvalid[i] = slave_rdec1[i]?s_axi_rvalid[i]:1'bz;
				assign slave_rvalid = trib_rvalid[i];
                assign trib_wvalid[i] = slave_wdec1[i]?s_axi_wvalid[i]:1'bz;
                assign slave_wvalid = trib_wvalid[i];
				assign trib_bvalid[i] = slave_wdec1[i]?s_axi_bvalid[i]:1'bz;
				assign slave_bvalid = trib_bvalid[i]; 
                assign trib_rready[i] = slave_rdec1[i]?s_axi_rready[i]:1'bz;
                assign slave_rready = trib_rready[i];
                assign trib_bready[i] = slave_wdec1[i]?s_axi_bready[i]:1'bz;
                assign slave_bready = trib_bready[i];
			end
		end
	endgenerate
	wire master_rready, master_bready, master_rvalid, master_bvalid;
	generate
		if(addressing) begin
			// Giant MUX
			assign master_rready = m_axi_rready[counter_rrequests];
			assign master_bready = m_axi_bready[counter_wrequests];
            assign master_rvalid = m_axi_rvalid[counter_rrequests];
            assign master_bvalid = m_axi_bvalid[counter_wrequests];
		end else begin
			// tri-state buff bus
			wire [masters-1:0] trib_rready;
			wire [masters-1:0] trib_bready;
            wire [masters-1:0] trib_rvalid;
            wire [masters-1:0] trib_bvalid;
			for (i = 0; i < masters; i = i + 1) begin 
                ...
                assign trib_rvalid[i] = dec_rrequests[i]?m_axi_rvalid[i]:1'bz;
                assign master_rvalid = trib_rvalid[i];
                assign trib_bvalid[i] = dec_wrequests[i]?m_axi_bvalid[i]:1'bz;
                assign master_bvalid = trib_bvalid[i];
			end
		end
	endgenerate
	wire fi_rrequests, fi_wrequests;
	assign fi_rrequests = master_rready & slave_rvalid & slave_rready & master_rvalid;
	assign fi_wrequests = master_bready & slave_bvalid & slave_bready & master_bvalid;
	...
	/*ila_2 ila_2_inst (
        ...
        .probe2(is_rrequests),
        .probe3(is_wrequests),
        .probe4(dec_rrequests),
        .probe5(dec_wrequests)
    );*/
